...
class RS_KGA0_att2_subexp1(nn.Module):
    def __init__(self, data_config, args_config):
        ...
        cs1 = np.around(0 * self.r_num + self.emb_size)
    ...
    def _init_weight(self):
        ...
        self.all_embed_cf = initializer(torch.empty(self.n_users + self.n_items, self.emb_size), gain=gain)
        self.all_embed = initializer(torch.empty(self.n_entities, int(self.n_relations * self.emb_size)), gain=gain)
...
class KGA00(nn.Module):
    ...
    def forward(self, graph, entity_emb):
        ...
        for i in range(self.n_relations):
            ...
            entity_emb_f = entity_emb_f + graph.nodes['item'].data['node0{}'.format(i)]
