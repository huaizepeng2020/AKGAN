...
class Aggregator_MAKG100(nn.Module):
    ...
    def forward(self, graph, graph_AU, entity_emb, user_emb, W_R, drop_out):
        ...
        graph_AU.apply_edges(self.cal_attribute1_att)
        # graph.update_all(
        #     dgl.function.copy_e('att', 'temp0'),
        #     dgl.function.sum('temp0', 'nodeatt'))
        # graph.apply_edges(self.cal_attribute2)
...
class GraphConv_KGRS(nn.Module):
    ...
    def forward(self, graph, graph_AU, graph_UIS, all_embed, W_R, drop_out=False):
        ...
        if drop_out:
            ...
            # mask = torch.zeros(num_edge1, 1)
            # mask[random_indices] = mask[random_indices] + 1
            # mask0 = mask.to(user_embed.device)
        ...
        for j in range(self.n_hops):
            entity_f_agg, user_f_agg = self.convs_ui0[j](graph, graph_AU, entity_f_agg, user_f_agg, W_R, drop_out)
            # if True:
            #     entity_f_agg = self.dropout(entity_f_agg)
