...
class Aggregator_I2U(nn.Module):
    ...
    def forward(self, graph_UIS, entity_embed, user_embed):
        ...
        #     dgl.function.sum('temp2', 'node1'), etype=1)

        # graph_UIS.update_all(
        #     dgl.function.copy_u('node1', 'temp3'),
        #     dgl.function.sum('temp3', 'node2'), etype=1)

        # user_emb_f = graph_UIS.nodes['user'].data['node1']

        # deg = graph_UIS.nodes['user'].data['deg']
        # user_emb_f = graph_UIS.nodes['user'].data['node1'] * graph_UIS.nodes['user'].data['node1']
        # user_emb_f1 = graph_UIS.nodes['user'].data['node2']
        # user_emb_f = user_emb_f - user_emb_f1
        # user_emb_f = user_emb_f / (deg * (deg - 1 + 0.0001))
        # user_emb_f = F.normalize(user_emb_f, dim=1)

        # user_emb_f = graph_UIS.nodes['user'].data['node1'] + graph_UIS.nodes['user'].data['node0']
        # user_emb_f = graph_UIS.nodes['user'].data['node1'] + graph_UIS.nodes['user'].data['node1'] * \
        #              graph_UIS.nodes['user'].data['node1']
        user_emb_f = graph_UIS.nodes['user'].data['node0']
        user_emb_f = torch.sum(user_emb_f, dim=1)
        # a1 = graph_UIS.nodes['user'].data['node0']
        # user_emb_f = user_emb_f + graph_UIS.nodes['user'].data['node0']
...
class GraphConv_UIFGAN(nn.Module):
    ...
    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, num_like, n_epoch):
        ...
        self.tri_embed = tri_edge_embed_r(n_users=n_users, n_entities=n_entities, n_items=n_items,
    ...
    def forward(self, graph, graph_nor, graph_UIS, all_embed, W_R):
        ...
        entity_f0 = torch.cat(entity_f0, dim=2). \
            reshape(self.n_entities, self.n_relations, (1 + self.n_hops), -1)
        entity_f0 = torch.sum(entity_f0, dim=2)
        ...
        user_embed_agg = self.item2user(graph_UIS, entity_f0, user_embed)
