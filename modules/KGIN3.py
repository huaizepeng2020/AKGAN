...
class GraphConv_UIFGAN(nn.Module):
    ...
    def forward(self, graph, graph_UIS, all_embed, W_R):
        user_embed = all_embed[:self.n_users, :]
        ...
        for i in range(self.num_like):
            ...
            entity_att_hop_all = []
            entity_att_hop_all.append(torch.ones(self.n_entities, 1).to(user_embed.device))
            ...
            for j in range(self.n_hops):
                ...
                # R_att_f = torch.cosine_similarity(entity_f_agg.unsqueeze(1), W_R.unsqueeze(0), dim=2)
                R_att_f = torch.mm(entity_f_agg, W_R.permute(1, 0))
                entity_att_hop_all.append(torch.mm(nn.Softmax(dim=1)(R_att_f), exp_deg[i].unsqueeze(1)))
            ...
            entity_att_hop_all = torch.cat(entity_att_hop_all, dim=1)
            entity_att_hop_all = nn.Softmax(dim=1)(entity_att_hop_all) * (1 + self.num_like)
            # entity_f1 = torch.sum(entity_f1, dim=1)
            entity_f1 = torch.matmul(entity_att_hop_all.unsqueeze(1), entity_f1).squeeze(1)
...
class RS_UIFKAN(nn.Module):
    ...
    def forward(self, batch, graph, graph_UIS, graph_r):
        ...
        #                                       self.W_R,
        #                                       self.common_like, self.user_like)
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_UIS, self.all_embed, self.W_R)
