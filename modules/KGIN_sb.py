...
class Aggregator_MAKG1(nn.Module):
    ...
    def forward(self, graph, entity_emb, W_R):
        ...
        # graph.update_all(
        #     dgl.function.copy_e('att', 'temp0'),
        #     dgl.function.sum('temp0', 'nodeatt'))
        # graph.apply_edges(self.cal_attribute2)
        # graph.update_all(
        #     self.e_mul_e,
        #     dgl.function.sum('nei', 'node'))

        graph.update_all(
            dgl.function.copy_e('emb', 'temp'),
            dgl.function.mean('temp', 'node'))

        # entity_emb_f = F.normalize(graph.ndata['node'], dim=1) \
        #                + 0.1 * F.normalize(graph.ndata['node'] * graph.ndata['node'], dim=1)

        # graph.update_all(
        #     dgl.function.copy_u('node', 'temp'),
        #     dgl.function.mean('temp', 'node'))
        entity_emb_f = graph.ndata['node']
        entity_emb_f = F.normalize(entity_emb_f, dim=1)

        return entity_emb_f


class Aggregator_MAKG100(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(Aggregator_MAKG100, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
        ...
        self.drop_edge = True
        self.drop_edge_rate = 0.7

    def cal_attribute1(self, edges):
        edge_emb = self.W_r[edges.data['type']] * edges.src['node']
        return {'emb': edge_emb}

    def cal_attribute1_att(self, edges):
        att = torch.exp(torch.sum(edges.src['node'] * edges.dst['node'], dim=1))
        return {'att': att}
    ...
    def forward(self, graph, graph_AU, graph_UIS, entity_emb, user_emb, W_R, drop_out):
        self.drop_edge = drop_out
        self.W_r = W_R
        ...
        """agg user"""
        # graph_AU = graph_AU.local_var()
        # graph_AU.ndata['node'] = torch.cat([entity_emb, user_emb], dim=0)
        # graph_AU.apply_edges(self.cal_attribute1)
        # graph_AU.update_all(
        #     dgl.function.copy_e('emb', 'temp'),
        #     dgl.function.mean('temp', 'node'))
        # user_emb_f = graph_AU.ndata['node'][self.n_entities:, :]

        graph_UIS = graph_UIS.local_var()
        graph_UIS.nodes['item'].data['node'] = entity_emb[:self.n_items]
        graph_UIS.nodes['item'].data['node0'] = graph.ndata['node'][:self.n_items]
        graph_UIS.nodes['user'].data['node'] = user_emb

        # graph_UIS.apply_edges(self.cal_attribute1_att, etype=1)
        # graph_UIS.update_all(
        #     dgl.function.copy_e('att', 'temp1'),
        #     dgl.function.sum('temp1', 'nodeatt'), etype=1)
        # graph_UIS.apply_edges(self.cal_attribute2, etype=1)
        # graph_UIS.update_all(
        #     dgl.function.u_mul_e('node', 'att1', 'temp2'),
        #     dgl.function.sum('temp2', 'node'), etype=1)

        graph_UIS.update_all(
            dgl.function.copy_u('node', 'temp2'),
            dgl.function.mean('temp2', 'node'), etype=1)
        user_emb_f0 = graph_UIS.nodes['user'].data['node']

        graph_UIS.update_all(
            dgl.function.copy_u('node0', 'temp3'),
            dgl.function.sum('temp3', 'node2'), etype=1)
        user_emb_f1 = graph_UIS.nodes['user'].data['node2']

        # user_emb_f = user_emb_f + user_emb_f0
        # if self.drop_edge:
        #     user_emb_f = self.dropout(user_emb_f)
        # user_emb_f = F.normalize(user_emb_f, dim=1)

        if self.drop_edge:
            user_emb_f1 = self.dropout(user_emb_f1)
            user_emb_f0 = self.dropout(user_emb_f0)

        user_emb_f1 = F.normalize(user_emb_f1, dim=1)
        user_emb_f0 = F.normalize(user_emb_f0, dim=1)
        user_emb_f = user_emb_f1 + user_emb_f0

        return entity_emb_f, user_emb_f


class Aggregator_MAKG100_1(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, n_like, dim):
        super(Aggregator_MAKG100_1, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_attribute1(self, edges):
        # edge_emb = edges.src['node']
        edge_emb = edges.src['node'] * self.W_r[edges.data['type']].unsqueeze(1)
        # edge_emb = edges.src['node'] * F.normalize(self.W_r[edges.data['type']], dim=1).unsqueeze(1)
        # edge_emb = F.normalize(edge_emb, dim=2)
        # edge_emb = edge_emb * edges.src['norm']

        edge_emb = edge_emb * self.like[:, edges.data['type']].t().unsqueeze(2)

        # edge_emb1 = F.normalize(edge_emb1, dim=2)

        # edge_emb = self.W_r[edges.data['type']] * edges.src['node'] * self.like[edges.data['type']]
        return {'emb': edge_emb}

    def cal_attribute1_att(self, edges):
        # att = torch.exp(torch.sum(edges.src['node'] * edges.dst['node'].unsqueeze(1), dim=2))

        # att = torch.exp(torch.cosine_similarity(edges.src['node'], edges.dst['node'].unsqueeze(1), dim=2))
        # att = nn.Softmax(dim=-1)(att)
        # emb = torch.matmul(att.unsqueeze(1), edges.src['node']).squeeze(1)

        emb = torch.matmul(edges.dst['att'].unsqueeze(1), edges.src['node']).squeeze(1)
        # emb = edges.src['node'].squeeze(1)
        return {'emb': emb}
    ...
    def forward(self, graph, graph_UIS, entity_emb, user_emb, W_R, like, like0, drop_out):
        self.drop_edge = drop_out

        self.W_r = W_R
        self.like = nn.Softmax(dim=-1)(like) * self.n_relations
        # self.like = nn.Softmax(dim=-1)(like) * self.n_relations * 0 + 1
        ...
        # graph.ndata['norm'] = torch.norm(entity_emb, dim=2, keepdim=True)
        ...
        entity_emb_f1 = graph.ndata['node']
        # entity_emb_f1 = F.normalize(graph.ndata['node'], dim=2)

        """agg user"""
        graph_UIS = graph_UIS.local_var()
        graph_UIS.nodes['item'].data['node'] = entity_emb_f1[:self.n_items]
        # graph_UIS.nodes['user'].data['node'] = user_emb

        user_att1 = torch.mm(user_emb, like0.t())
        user_att1 = nn.Softmax(dim=-1)(user_att1) * entity_emb_f1.shape[1]
        # user_att1 = nn.Softmax(dim=-1)(user_att1) * 0 + 1
        # graph_UIS.nodes['user'].data['att'] = user_att1

        # graph_UIS.apply_edges(self.cal_attribute1_att, etype=1)
        # graph_UIS.update_all(
        #     dgl.function.copy_e('emb', 'temp1'),
        ...
        user_emb_f = graph_UIS.nodes['user'].data['node']
        user_emb_f = torch.matmul(user_att1.unsqueeze(1), user_emb_f).squeeze(1)
        #
        # entity_emb_f1 = F.normalize(entity_emb_f1, dim=2)
        # user_emb_f = F.normalize(user_emb_f, dim=1)

        return entity_emb_f1, user_emb_f


class Aggregator_MAKG100_mul(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(Aggregator_MAKG100_mul, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_attribute1_att(self, edges):
        edge_emb = self.W_r[edges.data['type']] * edges.src['node'] * edges.data['weight'].unsqueeze(1)
        # att = torch.exp(torch.cosine_similarity(edge_emb, edges.dst['node'], dim=1)) * edges.data['weight'].unsqueeze(1)
        # att = torch.exp(torch.sum(edge_emb * edges.dst['node'], dim=1)) * edges.data['weight'].unsqueeze(1)

        # edges.data['weight'].min()
        # edge_emb = edge_emb * edges.data['weight'].unsqueeze(1)
        return {'emb': edge_emb}
        # return {'emb': edge_emb, 'att': att}
    ...
    def forward(self, graph, graph_AU, graph_UI, entity_emb, user_emb, W_R, drop_out):
        self.drop_edge = drop_out
        self.W_r = W_R
        ...
        """agg user"""
        user_emb_f = []
        for t in range(len(graph_AU)):
            graph_AU[t] = graph_AU[t].local_var()
            graph_AU[t].ndata['node'] = torch.cat([entity_emb, user_emb], dim=0)
            graph_AU[t].apply_edges(self.cal_attribute1_att)
            graph_AU[t].update_all(
                dgl.function.copy_e('emb', 'temp'),
                dgl.function.sum('temp', 'node'))
            user_emb_f_c = graph_AU[t].ndata['node'][self.n_entities:, :]

            graph_UI[t] = graph_UI[t].local_var()
            graph_UI[t].nodes['item'].data['node'] = entity_emb[:self.n_items]
            graph_UI[t].update_all(
                dgl.function.copy_u('node', 'temp1'),
                dgl.function.mean('temp1', 'node'), etype=1)
            user_emb_f0 = graph_UI[t].nodes['user'].data['node']

            user_emb_f_c1 = user_emb_f_c + user_emb_f0

            if self.drop_edge:
                user_emb_f_c = self.dropout(user_emb_f_c)

            user_emb_f.append(F.normalize(user_emb_f_c, dim=1))
        user_emb_f = torch.cat(user_emb_f, dim=1).reshape(self.n_users, len(graph_AU), -1)

        att = torch.sum(user_emb.unsqueeze(1) * user_emb_f, dim=2)
        att = nn.Softmax(dim=1)(att)
        user_emb_f = torch.matmul(att.unsqueeze(1), user_emb_f).squeeze(1)
...
class tri_edge_embed(nn.Module):
    ...
    def cal_attribute1(self, edges):
        ...
        edge_emb = edges.src['node'] * self.W_r[edges.data['type']]
        # edge_emb = edges.src['node'] * F.normalize(self.W_r[edges.data['type']], dim=1)
        # edge_emb = F.normalize(edge_emb, dim=1)

        edge_emb = edge_emb.unsqueeze(1).expand(len(edges), self.like.shape[0], self.dim) \
                   * self.like[:, edges.data['type']].t().unsqueeze(2)
    ...
    def forward(self, graph, entity_emb, W_R, user_like):
        ...
        self.W_r = W_R
        # self.like = nn.Softmax(dim=-1)(user_like) * self.n_relations
        self.like = nn.Softmax(dim=-1)(user_like) * self.n_relations
        ...
        # entity_emb_new = F.normalize(graph.ndata['node1'], dim=2)
        # entity_emb_new = entity_emb_new.unsqueeze(1)
...
class tri_edge_embed_r(nn.Module):
    ...
    def cal_attribute1(self, edges):
        ...
        # edge_emb = edges.src['node'] * self.W_r * edges.dst['node']
    ...
    def forward(self, graph, entity_emb, W_R):
        ...
        for i in range(self.n_relations):
            ...
            graph.update_all(
                dgl.function.copy_u('node', 'send'),
                dgl.function.mean('send', 'node{}'.format(i)), etype=i)
            # graph.nodes['item'].data['node{}'.format(i)] = self.W_r * graph.nodes['item'].data['node{}'.format(i)]
            graph.nodes['item'].data['node{}'.format(i)] = \
                torch.mm(graph.nodes['item'].data['node{}'.format(i)], self.W_r)
            # graph.nodes['item'].data['node{}'.format(i)] = \
...
class Aggregator_I2U(nn.Module):
    ...
    def cal_attribute_UI0(self, edges):
        # atr1 = torch.exp(torch.sum(edges.src['node'] * edges.dst['node'].unsqueeze(1), dim=2))
        atr1 = torch.exp(torch.cosine_similarity(edges.src['node'], edges.dst['node'].unsqueeze(1), dim=2))
        # atr1 = torch.exp(torch.cosine_similarity(edges.src['node'], edges.dst['node'], dim=1))
        # atr1 = torch.sum(edges.src['node'] * edges.dst['node'].unsqueeze(1), dim=2)
        # atr1 = torch.exp(torch.sum(edges.src['node'] * edges.dst['node'], dim=1))
        return {'att': atr1}
    ...
    def forward(self, graph_UIS, entity_embed0, entity_embed, user_embed):
        ...
        # graph_UIS.nodes['item'].data['node1'] = entity_embed[:self.n_items] * entity_embed[:self.n_items]
        ...
        # graph_UIS.update_all(
        #     dgl.function.copy_e('att', 'temp1'),
        #     dgl.function.sum('temp1', 'sumatt'), etype=1)
        # graph_UIS.apply_edges(self.cal_attribute_UI1, etype=1)
        # graph_UIS.update_all(
        #     self.u_mul_e,
        #     dgl.function.sum('nei', 'node0'), etype=1)
        # user_att = graph_UIS.nodes['user'].data['sumatt']
        # user_att = F.normalize(user_att, dim=1, p=1) * entity_embed.shape[1]
        ...
        #
        # graph_UIS.update_all(
        #     dgl.function.copy_u('node0', 'temp4'),
        #     dgl.function.mean('temp4', 'node00'), etype=1)

        # user_embed_att = user_embed + graph_UIS.nodes['user'].data['node00']
        user_emb_f = graph_UIS.nodes['user'].data['node0']
        # user_att = torch.sum(user_emb_f * user_embed_att.unsqueeze(1), dim=2)
        # user_att = nn.Softmax(dim=1)(user_att) * self.n_relations

        # user_emb_f = torch.matmul(user_att.unsqueeze(1), user_emb_f).squeeze(1)
        user_emb_f = torch.sum(user_emb_f, dim=1)

        # user_emb_f = user_emb_f + graph_UIS.nodes['user'].data['node00']
...
class Aggregator_I2U0(nn.Module):
    ...
    def forward(self, graph_UIS, entity_embed0, entity_embed, user_embed, like0, drop_edge):
        """cal user"""
        graph_UIS = graph_UIS.local_var()
        graph_UIS.nodes['item'].data['node0'] = entity_embed0[:self.n_items]
        graph_UIS.nodes['item'].data['node'] = entity_embed[:self.n_items]
        ...
        user_att1 = torch.mm(user_embed, like0.t())
        user_att1 = nn.Softmax(dim=-1)(user_att1) * entity_embed.shape[1]
        # user_att1 = nn.Softmax(dim=-1)(user_att1) * 0 + 1
        graph_UIS.nodes['user'].data['att'] = user_att1
        ...
        graph_UIS.update_all(
            dgl.function.copy_u('node0', 'temp0'),
            dgl.function.mean('temp0', 'node0'), etype=1)

        user_emb_f0 = graph_UIS.nodes['user'].data['node0']
        user_emb_f = graph_UIS.nodes['user'].data['node']
        user_emb_f = torch.matmul(user_att1.unsqueeze(1), user_emb_f).squeeze(1)
...
class Aggregator_I2U1(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(Aggregator_I2U1, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def forward(self, graph_UIS, entity_embed, user_embed, like0, drop_edge):
        """cal user"""
        graph_UIS = graph_UIS.local_var()
        graph_UIS.nodes['item'].data['node'] = entity_embed[:self.n_items]
        graph_UIS.nodes['user'].data['node'] = user_embed
        ...
        user_att1 = torch.mm(user_embed, like0.t())
        user_att1 = nn.Softmax(dim=-1)(user_att1)
        user_att1 = nn.Softmax(dim=-1)(user_att1) * 0 + 0.5
        graph_UIS.nodes['user'].data['att'] = user_att1
...
class GraphConv_UIFGAN(nn.Module):
    ...
    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, num_like, n_epoch):
        ...
        self.tri_embed = tri_edge_embed(n_users=n_users, n_entities=n_entities, n_items=n_items,
        ...
        for j in range(n_hops):
            self.tri_embed_u.append(
                tri_edge_embed_att(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                   n_relations=n_relations, dim=dim))
        ...
        self.kmeans_r = KMeans(n_clusters=self.num_like)
        # self.kmeans_r = AffinityPropagation(preference=-50)
        self.kmeans_r.num = 1

        self.pos = nn.LeakyReLU()

    def forward(self, graph, graph_nor, graph_UIS, all_embed, W_R):
        user_embed = all_embed[:self.n_users, :]
        entity_embed = all_embed[self.n_users:self.n_users + self.n_entities, :]
        W_R = all_embed[self.n_users + self.n_entities:, :]

        # """cal edge embedding"""
        # entity_embed0 = self.tri_embed(graph_nor, entity_embed, W_R)
        # if True:
        #     entity_embed0 = self.dropout(entity_embed0)

        # entity_embed0 = F.normalize(entity_embed0)
        ...
        # entity_f0.append(entity_embed)
        # entity_f0.append(entity_embed0)
        # entity_f_agg = entity_embed0
        for j in range(self.n_hops):
            # entity_f_agg = self.convs_ui0[j](graph, entity_f_agg)
            entity_f_agg = self.convs_ui0[j](graph_nor, entity_f_agg, W_R)
        ...
        #         self.kmeans.num = 1
        #         self.kmeans = self.kmeans.fit(user_embed.detach().cpu().numpy())
        #         # self.kmeans = self.kmeans.fit(user_embed.detach().cpu().numpy() + user_embed.detach().cpu().numpy())
        #     self.kmeans.num += 1
        ...
        # self.out = torch.cosine_similarity(self.common_like.unsqueeze(1),
        #                                    self.common_like.unsqueeze(0), dim=2)
        """this"""
        ...
        # R_att = nn.Softmax(dim=1)(R_att)* self.n_relations
        # self.kmeans_r = self.kmeans_r.fit(R_att.detach().cpu().numpy())
        # # self.kmeans_r = AffinityPropagation(preference=-0.05)
        # # self.kmeans_r = self.kmeans_r.fit(R_att.detach().cpu().numpy()[:10000,:])
        # # self.kmeans_r.cluster_centers_indices_
        # exp_deg = torch.tensor(self.kmeans_r.cluster_centers_, device=user_embed.device)
        # """test"""
        ...
        # entity_emb_all1 = []
        # for i in range(self.num_like):
        #     entity_embed_c = self.tri_embed_u[i](graph, entity_embed, W_R, self.common_like[i])
        #     entity_embed_c = F.normalize(entity_embed_c)
        #
        #     entity_f1 = []
        #     entity_f1.append(entity_embed_c)
        #     entity_f_agg = entity_embed_c
        #     for j in range(self.n_hops):
        #         entity_f_agg = self.convs_ui0[j](graph, entity_f_agg)
        #         if True:
        #             entity_f_agg = self.dropout(entity_f_agg)
        ...
        """agg user from diffirent atr"""
        # user_embed_agg = self.item2user(graph_UIS, entity_emb_all1, user_embed)
        user_embed_agg = self.item2user(graph_UIS, entity_embed, entity_f0, user_embed)

        """OUTPUT"""
        # user_emb_f = user_embed + user_embed_agg
        # entity_emb_f = torch.mean(entity_emb_all1, dim=1)

        # user_emb_f = user_embed_agg
        user_emb_f = user_embed_agg + user_embed
        # entity_emb_f = entity_f0
        # entity_emb_f = torch.mean(entity_f0, dim=1)
        entity_emb_f = torch.sum(entity_f0, dim=1)
        # entity_emb_f = torch.sum(entity_f0, dim=1) + entity_embed
...
class GraphConv_KGRS(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, num_like, n_epoch):
        super(GraphConv_KGRS, self).__init__()
        ...
        for j in range(n_hops):
            self.convs_ui0.append(
                Aggregator_MAKG100(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                   n_relations=n_relations,
                                   dim=dim))
    ...
    def forward(self, graph, graph_AU, graph_UIS, all_embed, W_R, drop_out=False):
        user_embed = all_embed[:self.n_users, :]
        entity_embed = all_embed[self.n_users:self.n_users + self.n_entities, :]
        # W_R = all_embed[self.n_users + self.n_entities:, :]
        ...
        """agg"""
        if drop_out:
            num_edge1 = graph.num_edges()
            random_indices = np.random.choice(num_edge1, size=int(num_edge1 * self.drop_edge_rate), replace=False)
            graph = dgl.edge_subgraph(graph.cpu(), random_indices, preserve_nodes=True).to(graph.device)

            # num_edge1 = graph_AU.num_edges()
            # random_indices = np.random.choice(num_edge1, size=int(num_edge1 * self.drop_edge_rate), replace=False)
            # graph_AU = dgl.edge_subgraph(graph_AU.cpu(), random_indices, preserve_nodes=True).to(graph.device)

            num_edge1 = graph_UIS.num_edges()
            random_indices = np.random.choice(num_edge1, size=int(num_edge1 * self.drop_edge_rate), replace=False)
            graph_UIS = dgl.edge_subgraph(graph_UIS.cpu(), random_indices, preserve_nodes=True).to(graph.device)

        user_f0 = []
        entity_f0 = []

        user_f0.append(user_embed)
        user_f_agg = user_embed

        entity_f0.append(entity_embed)
        entity_f_agg = entity_embed

        for j in range(self.n_hops):
            entity_f_agg, user_f_agg = self.convs_ui0[j](graph, graph_AU, graph_UIS, entity_f_agg, user_f_agg, W_R,
                                                         drop_out)
            # if True:
            #     entity_f_agg = self.dropout(entity_f_agg)
            #     user_f_agg = self.dropout(user_f_agg)
            entity_f0.append(entity_f_agg)
            user_f0.append(user_f_agg)

        entity_f0 = torch.cat(entity_f0, dim=1). \
            reshape(self.n_entities, (1 + self.n_hops), -1)
        entity_f0 = torch.sum(entity_f0, dim=1)

        user_f0 = torch.cat(user_f0, dim=1). \
            reshape(self.n_users, (1 + self.n_hops), -1)
        user_f0 = torch.sum(user_f0, dim=1)
        """agg"""
...
class GraphConv_KGRS1(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, num_like, n_epoch):
        super(GraphConv_KGRS1, self).__init__()
        ...
        for j in range(n_hops):
            self.convs_ui0.append(
                Aggregator_MAKG100_1(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                     n_relations=n_relations, n_like=num_like,
                                     dim=dim))
        ...
        self.pos = nn.LeakyReLU()
        self.drop_edge_rate = 0.5

    def forward(self, graph, graph_AU, graph_UIS, all_embed, W_R, user_like, user_like0, drop_out=False):
        # all_embed = F.normalize(all_embed, dim=1)
        ...
        entity_embed0 = self.tri_embed(graph, entity_embed, W_R, user_like)
        user_embed0 = self.item2user0(graph_UIS, entity_embed, entity_embed0, user_embed, user_like0, True)

        if drop_out:
            entity_embed0 = self.dropout(entity_embed0)
            user_embed0 = self.dropout(user_embed0)

        entity_embed0 = F.normalize(entity_embed0, dim=2)
        user_embed0 = F.normalize(user_embed0, dim=1)

        # if drop_out:
        #     user_embed0 = self.item2user0(graph_UIS, entity_embed, entity_embed0, user_embed, user_like0, True)
        # else:
        #     user_embed0 = self.item2user0(graph_UIS, entity_embed, entity_embed0, user_embed, user_like0, False)

        """user agg first"""
        user_f0 = user_embed
        user_f0 = torch.add(user_f0, user_embed0)

        """item agg first"""
        entity_f0 = entity_embed.unsqueeze(1).expand(self.n_entities, self.num_like, self.dim)
        entity_f0 = torch.add(entity_f0, entity_embed0)

        """agg"""
        for j in range(self.n_hops):
            entity_embed0, user_embed0 = self.convs_ui0[j](graph, graph_UIS, entity_embed0, user_embed0, W_R,
                                                           user_like, user_like0,
                                                           drop_out)
            if drop_out:
                entity_embed0 = self.dropout(entity_embed0)
                user_embed0 = self.dropout(user_embed0)
...
class GraphConv_KGRS_mul(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, num_like, n_epoch):
        super(GraphConv_KGRS_mul, self).__init__()
        ...
        for j in range(n_hops):
            self.convs_ui0.append(
                Aggregator_MAKG100_mul(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                       n_relations=n_relations,
                                       dim=dim))
    ...
    def forward(self, graph, graph_AU, graph_UIS, all_embed, W_R, drop_out=False):
        ...
        if drop_out:
            ...
            for t in range(len(graph_AU)):
                num_edge1 = graph_AU[t].num_edges()
                random_indices = np.random.choice(num_edge1, size=int(num_edge1 * self.drop_edge_rate), replace=False)
                graph_AU1.append(
                    dgl.edge_subgraph(graph_AU[t].cpu(), random_indices, preserve_nodes=True).to(graph.device))
        else:
            for t in range(len(graph_AU)):
                graph_AU1.append(graph_AU[t].cpu().to(graph.device))
        ...
        for j in range(self.n_hops):
            entity_f_agg, user_f_agg = self.convs_ui0[j](graph, graph_AU1, graph_UIS, entity_f_agg, user_f_agg, W_R,
                                                         drop_out)
            # if True:
            #     entity_f_agg = self.dropout(entity_f_agg)
            #     user_f_agg = self.dropout(user_f_agg)
...
class RS_UIFKAN(nn.Module):
    ...
    def _init_weight(self):
        ...
        # self.W_R = initializer(torch.empty(self.n_relations, self.emb_size, self.emb_size), gain=gain)
        self.W_R = initializer(torch.empty(self.n_relations, self.emb_size), gain=gain)
        # self.user_like = initializer(torch.empty(self.num_like, self.emb_size), gain=gain)
        # self.common_like = initializer(torch.empty(self.num_like, self.emb_size), gain=gain1)

    def _init_model(self):
        return GraphConv_KGRS(dim=self.emb_size,
                              n_hops=self.context_hops,
                              n_users=self.n_users,
                              n_items=self.n_items,
                              n_entities=self.n_entities,
    ...
    def forward(self, batch, graph, graph_AU, graph_UIS):
        user = batch['users']
        pos_item = batch['pos_items']
        neg_item = batch['neg_items']
        ...
        #                                       self.W_R,
        #                                       self.common_like, self.user_like)
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_AU, graph_UIS, self.all_embed, self.W_R, True)
    ...
    def generate(self, graph, graph_AU, graph_UIS):
        # return self.gcn(graph, graph_UIS, graph_r, self.all_embed,
        #                 self.W_R,
        #                 self.common_like, self.user_like)
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_AU, graph_UIS, self.all_embed, self.W_R, False)
...
class RS_UIFKAN1(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_UIFKAN1, self).__init__()
        ...
        self.W_R = nn.Parameter(self.W_R)
        self.user_like = nn.Parameter(self.user_like)
        self.user_like0 = nn.Parameter(self.user_like0)
        # self.common_like = nn.Parameter(self.common_like)
    ...
    def _init_weight(self):
        ...
        # self.W_R = initializer(torch.empty(self.n_relations, self.emb_size, self.emb_size), gain=gain)
        self.W_R = initializer(torch.empty(self.n_relations, self.emb_size), gain=gain)
        self.user_like = initializer(torch.empty(self.num_like, self.n_relations), gain=gain)
        self.user_like0 = initializer(torch.empty(self.num_like, self.emb_size), gain=gain)
        # self.common_like = initializer(torch.empty(self.num_like, self.emb_size), gain=gain1)

    def _init_model(self):
        return GraphConv_KGRS1(dim=self.emb_size,
                               n_hops=self.context_hops,
                               n_users=self.n_users,
                               n_items=self.n_items,
                               n_entities=self.n_entities,
    ...
    def forward(self, batch, graph, graph_AU, graph_UIS):
        ...
        #                                       self.W_R,
        #                                       self.common_like, self.user_like)
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_AU, graph_UIS, self.all_embed, self.W_R,
                                              self.user_like, self.user_like0,
                                              True)
