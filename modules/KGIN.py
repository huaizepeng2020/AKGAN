...
class Aggregator_MAKG(nn.Module):
    ...
    def cal_attribute1(self, edges):
        ...
        # att_hop = self.r_att[self.r].expand(len(edges))
        # deg = torch.ones(len(edges)).to(att_hop.device)
        # att_tail = edges.src['node'] * self.W_r
        ...
        return {'atr': att_tail}
        # return {'atr': att_tail, 'att_hop': att_hop, 'deg': deg}
    ...
    def forward(self, graph, entity_emb, W_R, exp_deg):
        ...
        for i in range(self.n_relations):
            ...
            graph.update_all(
                ...
                dgl.function.sum('temp0', 'node{}'.format(i)), etype=i)

            # graph.update_all(
            #     dgl.function.copy_e('att_hop', 'temp1'),
            #     dgl.function.sum('temp1', 'att_hop{}'.format(i)), etype=i)

            # graph.update_all(
            #     dgl.function.copy_e('deg', 'temp2'),
            #     dgl.function.sum('temp2', 'deg{}'.format(i)), etype=i)
        ...
        # entity_att_hop = graph.nodes['item'].data['att_hop0']
        # for i in range(1, self.n_relations):
        #     entity_att_hop = entity_att_hop + graph.nodes['item'].data['att_hop{}'.format(i)]
        #
        # entity_deg = graph.nodes['item'].data['deg0']
        # for i in range(1, self.n_relations):
        #     entity_deg = entity_deg + graph.nodes['item'].data['deg{}'.format(i)]
        #
        # entity_att_hop = entity_att_hop / entity_deg

        # entity_emb_f = entity_emb_f
        # entity_emb_f = entity_emb_f + entity_emb

        return entity_emb_f
        # return entity_emb_f, entity_att_hop
...
class Aggregator_MAKG1(nn.Module):
    ...
    def cal_attribute1(self, edges):
        coe1 = torch.norm(edges.src['node'], dim=2, p=2).unsqueeze(2)
        edge_emb = edges.src['node'] * self.W_r
        coe2 = torch.norm(edge_emb, dim=2, p=2).unsqueeze(2)
        edge_emb = edge_emb / (0.000001 + coe2) * coe1
    ...
    def forward(self, graph, entity_emb, W_R):
        # self.disen_weight_att = nn.Softmax(dim=-1)(disen_weight_att) * disen_weight_att.shape[1]
        ...
        for i in range(self.n_relations):
            ...
            """mean_edge"""
            graph.update_all(
                dgl.function.copy_e('emb', 'send'),
                dgl.function.mean('send', 'node{}'.format(i)), etype=i)

            entity_emb_f = torch.add(entity_emb_f, graph.nodes['item'].data['node{}'.format(i)])

            # entity_emb_f = torch.add(entity_emb_f,
            #                          F.normalize(graph.nodes['item'].data['node{}'.format(i)], dim=1))
        ...
        #         dgl.function.mean('send', 'node{}'.format(i)), etype=i)
        #
        #     entity_emb_f.append(F.normalize(graph.nodes['item'].data['node{}'.format(i)], dim=1))
        #
        # entity_emb_f = torch.cat(entity_emb_f, dim=1).reshape(self.n_entities, self.n_relations, self.dim)
        #
        # a = torch.sum(entity_emb_f * entity_emb_f, dim=1)
        # a1 = torch.sum(entity_emb_f, dim=1) * torch.sum(entity_emb_f, dim=1)
        # a2 = F.normalize(a1 - a, dim=1)
        # entity_emb_f1 = torch.sum(entity_emb_f, dim=1)
        # entity_emb_f1 = entity_emb_f1 + a2
...
class KGE1(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(KGE1, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_attribute1(self, edges):
        coe1 = torch.norm(edges.src['node'], dim=1, p=2).unsqueeze(1)
        edge_emb = edges.src['node'] * self.W_r
        coe2 = torch.norm(edge_emb, dim=1, p=2).unsqueeze(1)
        edge_emb = edge_emb / (0.000001 + coe2) * coe1
    ...
    def forward(self, graph, entity_emb, W_R):
        ...
        #     entity_emb_f.append(graph.nodes['item'].data['node{}'.format(i)])
        # entity_emb_f = torch.cat(entity_emb_f, dim=1).reshape(self.n_entities, self.n_relations, self.dim)
        # entity_emb_f = torch.sum(entity_emb_f, dim=1)
        return entity_emb_f


class KGE11(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(KGE11, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def forward(self, graph, entity_emb, W_R):
        ...
        for i in range(self.n_relations):
            ...
            graph.nodes['item'].data['node{}'.format(i)] = F.normalize(graph.nodes['item'].data['node{}'.format(i)],
                                                                       dim=1)
...
class Aggregator_MAKG2(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(Aggregator_MAKG2, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_attribute1(self, edges):
        edge_emb = self.W_r[edges.data['type']] * edges.src['node']

        att = torch.exp(torch.cosine_similarity(edge_emb, edges.dst['node'], dim=1))
        return {'emb': edge_emb, 'att': att}
...
class tri_edge_embed(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(tri_edge_embed, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_attribute1(self, edges):
        edge_emb = edges.src['node'] * self.W_r[edges.data['type']] * edges.dst['node']
        edge_emb1 = edges.src['node'] * self.W_r[edges.data['type']]

        att = torch.exp(
            torch.cosine_similarity(edges.dst['node'], edge_emb, dim=1))
        return {'emb': edge_emb, 'emb1': edge_emb1, 'att': att}

    def cal_attribute11(self, edges):
        edge_emb = torch.matmul(
            edges.src['node'].unsqueeze(1), self.W_r[edges.data['type']]).squeeze(1)
    ...
    def e_mul_e(self, edges):
        att = edges.data['emb'] * edges.data['att1'].unsqueeze(1)
    ...
    def forward(self, graph, entity_emb, W_R):
        ...
        graph.apply_edges(self.cal_attribute11)

        # graph.update_all(
        #     dgl.function.copy_e('att', 'temp1'),
        #     dgl.function.sum('temp1', 'nodeatt'))
        # graph.apply_edges(self.cal_attribute2)
        # graph.update_all(
        #     self.e_mul_e,
        #     dgl.function.sum('nei', 'node1'))

        graph.update_all(
            dgl.function.copy_e('emb', 'temp0'),
            dgl.function.mean('temp0', 'node1'))
        # graph.update_all(
        #     dgl.function.copy_e('emb1', 'temp1'),
        #     dgl.function.mean('temp1', 'node2'))
        # order2_emb = F.normalize(entity_emb * graph.ndata['node2'] * graph.ndata['node2'], dim=1)

        # entity_emb_new = F.normalize(graph.ndata['node1'], dim=1)

        # entity_emb_new = F.normalize(graph.ndata['node1'], dim=1) \
        #                  + F.normalize(graph.ndata['node1'] * graph.ndata['node1'], dim=1)

        # entity_emb_new = F.normalize(graph.ndata['node1'], dim=1) + order2_emb

        entity_emb_new = F.normalize(graph.ndata['node1'], dim=1) + \
                         F.normalize(graph.ndata['node1'] * graph.ndata['node1'], dim=1) \
                         + entity_emb

        return entity_emb_new


class tri_edge_embed_r(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(tri_edge_embed_r, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def forward(self, graph, entity_emb, W_R, disen_weight_att):
        graph = graph.local_var()
        graph.nodes['item'].data['node'] = entity_emb
        ...
        for i in range(self.n_relations):
            ...
            # graph.nodes['item'].data['node{}'.format(i)] = \
            #     F.normalize(graph.nodes['item'].data['node{}'.format(i)], dim=1)
        ...
        entity_emb_f = torch.matmul(disen_weight_att, entity_emb_f)

        return entity_emb_f


class KGA0(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(KGA0, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
...
class KGA001(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag1):
        super(KGA001, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def forward(self, graph, entity_emb, W_R):
        ...
        # entity_emb0 = entity_emb[:, self.dim_flag1[-1]:]

        entity_emb_f = []
        # entity_emb_f.append(entity_emb0)
        for i in range(self.n_relations):
            # if i in graph._etypes:
            self.W_r = W_R[i].unsqueeze(0)
            self.r = i
...
class KGA000(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag1):
        super(KGA000, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
...
class KGE0(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(KGE0, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
...
class KGE10(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(KGE10, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
...
class tri_edge_embed_att(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(tri_edge_embed_att, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_attribute1(self, edges):
        ...
        att = torch.exp(torch.cosine_similarity(edge_emb, self.user_like, dim=1))
        return {'emb': edge_emb, 'att': att}

    def cal_attribute2(self, edges):
        att = edges.data['att'] / edges.dst['node_att']
        return {'att1': att}

    def dgl_e_mul_e(self, edges):
        att = edges.data['emb'] * edges.data['att1'].unsqueeze(1)
        return {'nei': att}

    def forward(self, graph, entity_emb, W_R, user_like):
        graph = graph.local_var()
        graph.ndata['node'] = entity_emb

        self.W_r = W_R
        self.user_like = user_like.unsqueeze(0)

        graph.apply_edges(self.cal_attribute1)

        graph.update_all(
            dgl.function.copy_e('att', 'temp1'),
            dgl.function.sum('temp1', 'node_att'))
        ...
        entity_emb_new = graph.ndata['node1']
...
class Aggregator_I2U(nn.Module):
    ...
    def cal_attribute_UI0(self, edges):
        ...
        # atr1 = nn.Softmax(dim=-1)(atr1).unsqueeze(1) * edges.src['node'].shape[1]
    ...
    def forward(self, graph_UIS, entity_embed, user_embed, user_like):
        # entity_embed0 = torch.sum(entity_embed, dim=1)
        """cal user"""
        graph_UIS = graph_UIS.local_var()
        ...
        # graph_UIS.update_all(
        #     dgl.function.copy_e('atr', 'temp1'),
        #     dgl.function.mean('temp1', 'node0'), etype=1)
        ...
        # graph_UIS.update_all(
        #     dgl.function.copy_u('node0', 'temp3'),
        #     dgl.function.mean('temp3', 'node00'), etype=1)

        # user_emb_f0 = graph_UIS.nodes['user'].data['node00']
        user_emb_f = graph_UIS.nodes['user'].data['node0']

        # score = nn.Softmax(dim=1)(user_like).unsqueeze(1) * user_like.shape[1]

        # score_ = torch.mm(user_emb_f0, user_like.t())
        # score = nn.Softmax(dim=1)(score_).unsqueeze(1) * user_like.shape[0]
        #
        score = nn.Sigmoid()(user_like).unsqueeze(1)
        user_emb_f = torch.matmul(score, user_emb_f).squeeze(1)

        return user_emb_f


class KGA2(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag):
        super(KGA2, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
...
class KGA2_atr_de(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag, dim_flag1):
        super(KGA2_atr_de, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_user_att(self, edges):
        user_att = []
        for i in range(self.n_relations):
            user_att_r = torch.sum(
                edges.src['node'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
                edges.dst['node0'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1)
            user_att.append(user_att_r)
        user_att = torch.cat(user_att, dim=1)
        return {'att': user_att}

    def forward(self, graph_UIS, entity_embed0, entity_embed, user_embed, user_like):
        """cal user"""
        graph_UIS = graph_UIS.local_var()
        ...
        # user_att = []
        # for i in range(self.n_relations):
        #     user_att.append(torch.sum(
        #         torch.mean(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=0) *
        #         torch.mean(entity_embed[:self.n_items][:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=0)).unsqueeze(
        #         0))
        # user_att = torch.cat(user_att, dim=0).unsqueeze(0)
        # user_att = nn.ReLU()(user_att) + 1e-11
        #
        # graph_UIS.update_all(
        #     self.cal_user_att,
        #     dgl.function.mean('att', 'att'), etype=1)
        # score = graph_UIS.nodes['user'].data['att']
        # score = score / user_att
        # score = nn.Tanh()(score - 1).detach()
        # score = nn.ReLU()(score)
        ...
        # att = []
        # for i in range(self.n_relations):
        #     # att.append(torch.mm(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]],
        #     #                     user_like[:, self.dim_flag1[i]:self.dim_flag1[i + 1]].t()) / self.dim_flag[i])
        #     att.append(torch.sum(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
        #                          user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1) /
        #                self.dim_flag[i] *
        #                user_embed.shape[1])
        #     # att.append(torch.mm(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]],user_like[str(i)].t()))
        # att = torch.cat(att, dim=1)
        # # att = torch.mm(user_emb_f, user_like.t())
        # score = nn.Softmax(dim=-1)(att) * self.n_relations

        # score = nn.Sigmoid()(10 * nn.Softmax(dim=-1)(user_like)) * 2 - 1
        # score = nn.LeakyReLU()(user_like)
        score = nn.Softmax(dim=-1)(user_like)
        # score = nn.Softmax(dim=-1)(user_like) + 1
        # score = nn.Softmax(dim=-1)(user_like) * self.n_relations
        self.out = score
        a = []
        for i in range(self.n_relations):
            a.append(score[:, i].unsqueeze(1).expand(-1, self.dim_flag[i]))
...
class KGA2_atr(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag, dim_flag1):
        super(KGA2_atr, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
        ...
        #     nn.Linear(self.dim_flag1[-1] * 2, self.dim_flag1[-1]),
        #     nn.LeakyReLU(),
        #     nn.Linear(self.dim_flag1[-1], self.n_relations),
        #     nn.Sigmoid())

        # self.attention = nn.Sequential(
        #     nn.Linear(self.dim_flag1[-1] * 3, self.dim_flag1[-1]),
        #     nn.LeakyReLU(),
        #     nn.Linear(self.dim_flag1[-1], self.n_relations),
        #     nn.Softmax(dim=1))
        #
        # initializer = nn.init.xavier_uniform_
        # gain = 1.414
        # self.W = initializer(torch.empty(self.dim_flag1[-1], self.dim_flag1[-1]), gain=gain)
    ...
    def forward(self, graph_UIS, entity_embed0, entity_embed, user_embed, user_like):
        ...
        # graph_UIS.update_all(
        #     self.cal_cluster,
        #     dgl.function.mean('att_cluster', 'att_cluster'), etype=1)
        # score = graph_UIS.nodes['user'].data['att_cluster'].detach()
        """-----------cluster degree---------------"""

        # """-----------feature MLP attention---------------"""
        # score = self.attention(torch.cat([user_emb_f, user_emb_f * user_emb_f], dim=1))
        # """-----------feature MLP attention---------------"""

        # """-----------feature MLP attention---------------"""
        # att_input = user_emb_f * user_emb_f
        # att = []
        # for i in range(self.n_relations):
        #     att.append(torch.sum(att_input[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1))
        # att = torch.cat(att, dim=1)
        # score = nn.Softmax(dim=-1)(att) + 1
        #
        # """-----------feature MLP attention---------------"""
        ...
        # att_input = torch.cat(
        #     [user_emb_f, torch.mm(user_embed, self.W), user_emb_f * torch.mm(user_embed, self.W)], dim=1)
        # score = self.attention(att_input)

        # score = nn.Softmax(dim=-1)(torch.pow(torch.abs(torch.mm(user_emb_f, user_like.t())) + 1e-13, 0.5))
        # score = nn.Softmax(dim=-1)(torch.mm(user_emb_f, user_like.t()) / (math.pow(self.dim_flag1[-1], 0.5)))

        # att = []
        # for i in range(self.n_relations):
        #     # att.append(torch.sum(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
        #     #                      user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1) /
        #     #            math.pow(self.dim_flag[i], 0.5))
        #     att.append(torch.sum(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
        #                          user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1) *
        #                self.dim_flag1[-1] * self.dim_flag1[-1] / self.dim_flag[i] / self.dim_flag[i] / self.dk)
        #     # att.append(torch.sum(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
        #     #                      user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1) /
        #     #            self.dim_flag[i])
        #     # att.append(torch.sum(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
        ...
        for i in range(self.n_relations):
            ...
            # att.append(user_att / mean_att)
            # att.append(0.1 * user_att / mean_att)
            att.append(nn.ReLU()(user_att / mean_att - 1))
            # att.append(torch.log(user_att / mean_att))
            # att.append(torch.sum(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
            #                      user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1)/
            #            torch.sum(torch.mean(entity_embed[:self.n_items][:, self.dim_flag1[i]:self.dim_flag1[i + 1]]) *
            #                      user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1))
        ...
        # score = nn.Softmax(dim=-1)(torch.mm(user_embed, user_like.t()) / (math.pow(self.dim_flag1[-1], 0.5)))
...
class KGA2_atr2(nn.Module):
    ...
    def forward(self, graph_UIS, entity_embed0, entity_embed, user_embed):
        ...
        # att = torch.cat(att, dim=1)
        #
        # # att1 = nn.PReLU(1e3)(att - 1 / 1e3)
        # # att = F.normalize(att, dim=1, p=1)
        # score = nn.Softmax(dim=-1)(att)
        ...
        for i in range(self.n_relations):
            ...
            mean_att = nn.ReLU()(mean_att) + 1e-8
            # att.append(torch.log(user_att / mean_att))
            # att.append(nn.ReLU()(user_att / mean_att - 1) + 0.01)
            att.append(self.k_att * nn.ReLU()(user_att / mean_att - 1) + 0.01)
            # att.append(0.25*user_att / mean_att)
            # att.append(0.1 * user_att / mean_att)
            # att.append(torch.sum(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
            #                      user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1)/
            #            torch.sum(torch.mean(entity_embed[:self.n_items][:, self.dim_flag1[i]:self.dim_flag1[i + 1]]) *
...
class KGA2_atr0(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag, dim_flag1):
        super(KGA2_atr0, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def att0(self, edges):
        att = torch.exp(torch.sum(edges.src['node_{}'.format(self.r)] * edges.dst['node_{}'.format(self.r)], dim=1))
        return {'att0_{}'.format(self.r): att}

    def att1(self, edges):
        atr1 = edges.data['att0_{}'.format(self.r)] / edges.dst['node_att_{}'.format(self.r)]
        return {'att1_{}'.format(self.r): atr1}

    def u_mul_e(self, edges):
        atr1 = edges.src['node_{}'.format(self.r)] * edges.data['att1_{}'.format(self.r)].unsqueeze(1)
        return {'nei_{}'.format(self.r): atr1}
    ...
    def forward(self, graph_UIS, entity_embed0, entity_embed, user_embed, user_like):
        ...
        for i in range(self.n_relations):
            graph_UIS.nodes['item'].data['node_{}'.format(i)] = entity_embed[:self.n_items,
                                                                self.dim_flag1[i]:self.dim_flag1[i + 1]]
            graph_UIS.nodes['user'].data['node_{}'.format(i)] = user_embed[:, self.dim_flag1[i]:self.dim_flag1[i + 1]]

        for i in range(self.n_relations):
            self.r = i

            graph_UIS.apply_edges(self.att0, etype=1)

            graph_UIS.update_all(
                dgl.function.copy_e('att0_{}'.format(self.r), 'send'),
                dgl.function.sum('send', 'node_att_{}'.format(i)), etype=1)

            graph_UIS.apply_edges(self.att1, etype=1)

            graph_UIS.update_all(
                self.u_mul_e,
                dgl.function.sum('nei_{}'.format(self.r), 'node_att_f_{}'.format(i)), etype=1)

        user_emb_f = []
        for i in range(self.n_relations):
            user_emb_f.append(graph_UIS.nodes['user'].data['node_att_f_{}'.format(i)])
        user_emb_f = torch.cat(user_emb_f, dim=1)
...
class KGA2_atrcat(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag, dim_flag1):
        super(KGA2_atrcat, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def cal_user_att(self, edges):
        # src=edges.src['node'].cpu()
        # dst=edges.dst['node0'].cpu()
        user_att = []
        for i in range(self.n_relations):
            # user_att.append(
            ...
            user_att_r = torch.sum(
                edges.src['node'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
                edges.dst['node0'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1)
            # user_att_r0 = torch.mean(user_att_r)
            # user_att_r0 = torch.mean(F.normalize(user_att_r, dim=1))

            # user_att_r0 = torch.sum(
            #     torch.mean(edges.src['node'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=0) *
            #     torch.mean(edges.dst['node0'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=0))
            #
            # user_att_r = user_att_r / user_att_r0
            ...
            #         edges.src['node'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]] *
            #         edges.dst['node0'][:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1).unsqueeze(1))
            # user_att0.append(torch.mean(user_att[-1]))
        user_att = torch.cat(user_att, dim=1)
        # user_att = nn.Tanh()(user_att)
    ...
    def forward(self, graph_UIS, entity_embed0, entity_embed, user_like):
        """cal user"""
        graph_UIS = graph_UIS.local_var()
        ...
        for i in range(self.n_relations):
            # user_att.append(torch.sum(
            #     torch.mean(user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=0) *
            #     torch.mean(entity_embed[:self.n_items][:, self.dim_flag1[i]:self.dim_flag1[i + 1]],dim=0)).unsqueeze(0))
            user_att.append(torch.mm(
                user_emb_f[:, self.dim_flag1[i]:self.dim_flag1[i + 1]],
                torch.mean(entity_embed[:self.n_items][:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=0).unsqueeze(
                    0).t()))
        # user_att = torch.cat(user_att, dim=0).unsqueeze(0)
        user_att = torch.cat(user_att, dim=1)
        user_att = nn.ReLU()(user_att) + 1e-13
        ...
        score = score / user_att
        # score = nn.Tanh()(score - 1).detach()
        score = nn.Tanh()(torch.log(nn.ReLU()(score) + 1e-13) / math.log(10)).detach()
        score = nn.ReLU()(score)
        ...
        user_emb_f = torch.cat([user_emb_f, user_emb_f0], dim=1)

        return user_emb_f


class KGA2_atr0cat(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag, dim_flag1):
        super(KGA2_atr0cat, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
...
class KGA20(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim, dim_flag):
        super(KGA20, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def forward(self, graph_UIS, entity_embed, user_embed, user_like):
        ...
        # user_emb_f0 = []
        # user_emb_f0.append(torch.zeros(self.n_users, np.sum(self.dim_flag)).to(user_emb_f.device))
        # user_emb_f0.append(user_embed[:, np.sum(self.dim_flag):])
        # user_emb_f0 = torch.cat(user_emb_f0, dim=1)
        # user_emb_f = user_emb_f + user_emb_f0
        ...
        # for i in range(self.n_relations):
        #     a.append(score[:, i].unsqueeze(1).expand(-1, self.dim_flag[i]))
        # a.append(torch.ones(self.n_users, entity_embed.shape[1] - np.sum(self.dim_flag)).to(entity_embed.device))
        # score = torch.cat(a, dim=1)
        # user_emb_f = score * user_emb_f
...
class KGE2(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(KGE2, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
    ...
    def forward(self, graph_UIS, entity_embed, user_embed):
        ...
        graph_UIS.nodes['item'].data['node'] = entity_embed
        graph_UIS.nodes['user'].data['node'] = user_embed
        ...
        graph_UIS.update_all(
            dgl.function.copy_u('node', 'temp1'),
            dgl.function.mean('temp1', 'node0'), etype=2)

        user_emb_f = graph_UIS.nodes['user'].data['node0']
        entity_emb_f = graph_UIS.nodes['item'].data['node0']

        return entity_emb_f, user_emb_f


class KGE12(nn.Module):

    def __init__(self, n_users, n_entities, n_items, n_relations, dim):
        super(KGE12, self).__init__()
        self.n_users = n_users
        self.n_entities = n_entities
        self.n_items = n_items
        self.dim = dim
        ...
        self.means = []
        for i in range(self.n_users):
            self.means.append(KMeans(n_clusters=3))
    ...
    def forward(self, graph_UIS, user_dict, entity_embed, user_embed, user):

        users = []
        for i in user:
            a = entity_embed[user_dict[int(i)]]
            self.means[i] = self.means[i].fit(a.cpu().detach().numpy())
            labels = self.means[i].labels_
            userc = 0
            for j in range(3):
                idx = np.where(labels == j)[0]
                if len(idx) > 1:
                    userc = torch.add(userc, torch.mean(a[idx], dim=0, keepdim=True))
                elif len(idx) == 1:
                    userc = torch.add(userc, a[idx])
            users.append(userc)
        users = torch.cat(users, dim=0)
...
class GraphConv_UIFGAN(nn.Module):
    ...
    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, num_like, ratedrop):
        super(GraphConv_UIFGAN, self).__init__()

        self.tri_embed = KGA0(n_users=n_users, n_entities=n_entities, n_items=n_items,
                              n_relations=n_relations, dim=dim)
        ...
        self.item2user = KGA2(n_users=n_users, n_entities=n_entities, n_items=n_items,
                              n_relations=n_relations,
                              dim=dim)

    def forward(self, graph, graph_nor, graph_UIS, all_embed, W_R, user_like, dropout):
        user_embed = all_embed[:self.n_users, :]
        ...
        """cal edge embedding"""
        entity_embed0 = self.tri_embed(graph, entity_embed, W_R, disen_weight_att)

        if dropout:
            entity_embed0 = self.dropout(entity_embed0)

        # entity_res_emb = entity_embed
        entity_res_emb = entity_embed.unsqueeze(1).expand(entity_embed.shape[0], self.num_like, entity_embed.shape[1])
        entity_res_emb = torch.add(entity_res_emb, entity_embed0)

        # entity_res_emb = entity_embed0

        for j in range(self.n_hops):
            entity_embed0 = self.convs_ui0[j](graph, entity_embed0, W_R)
            if dropout:
                entity_embed0 = self.dropout(entity_embed0)
            entity_res_emb = torch.add(entity_res_emb, entity_embed0)

        """agg user from diffirent atr"""
        user_embed_agg = self.item2user(graph_UIS, entity_res_emb, user_embed, user_like)

        user_emb_f = user_embed_agg + user_embed

        entity_res_emb = torch.sum(entity_res_emb, dim=1)
        # entity_res_emb = torch.add(entity_res_emb, entity_embed)
...
class GraphConv_KGA(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, dim_flag, ratedrop):
        super(GraphConv_KGA, self).__init__()
        ...
        self.item2user = KGA2(n_users=n_users, n_entities=n_entities, n_items=n_items,
                              n_relations=n_relations,
                              dim=dim, dim_flag=dim_flag)

    def forward(self, graph, graph_UIS, all_embed, W_R, user_like, dropout):
        user_embed = all_embed[:self.n_users, :]
        entity_embed = all_embed[self.n_users:self.n_users + self.n_entities, :]

        if dropout:
            # random_indices0 = np.random.choice(self.n_relations,
            #                                    size=int(self.n_relations * self.ratedrop),
            #                                    replace=False)
            # drop_r = []
            # for ii in random_indices0:
            #     drop_r.append(('item', ii, 'item'))
            # graph = dgl.edge_type_subgraph(graph, drop_r)
        ...
        """cal edge embedding"""
        entity_embed0 = self.tri_embed(graph, entity_embed, W_R)
        ...
        # if dropout:
        #     mask = torch.zeros(self.n_users, self.n_relations).to(user_embed.device)
        #     a = []
        #     for i in range(self.n_relations):
        #         if i in random_indices0:
        #             mask[:, i] = 1
        #         a.append(mask[:, i].unsqueeze(1).expand(-1, self.dim_flag[i]))
        #     mask = torch.cat(a, dim=1)
        #     user_emb_f = mask * user_emb_f
...
class GraphConv_KGA0(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, dim_flag, ratedrop):
        super(GraphConv_KGA0, self).__init__()
    ...
    def forward(self, graph, graph_UIS, all_embed, W_R, user_like, dropout):
        ...
        # entity_embed = []
        # for i in range(self.n_relations):
        #     user_embed.append(all_embed[i][:self.n_users, :])
        #     entity_embed.append(all_embed[i][self.n_users:self.n_users + self.n_entities, :])
        user_embed = all_embed[:self.n_users, :]
        entity_embed = all_embed[self.n_users:self.n_users + self.n_entities, :]
...
class GraphConv_KGA0_atr(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, dim_flag, ratedrop):
        super(GraphConv_KGA0_atr, self).__init__()
        ...
        self.item2user = KGA2_atr(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                  n_relations=n_relations,
                                  dim=dim, dim_flag=dim_flag, dim_flag1=self.dim_flag1)
    ...
    def forward(self, graph, graph_UIS, all_embed, W_R, user_like, dropout):
        ...
        # """---------------mask-------------------"""
        # mask = torch.zeros(self.n_entities, self.n_relations)
        # a = []
        # for j in range(self.n_relations):
        #     idx = torch.sum(entity_res_emb[:, self.dim_flag1[j]:self.dim_flag1[j + 1]], dim=1)
        #     idx1 = torch.where(idx != 0)[0]
        #     mask[idx1, j] = 1
        #     a.append(mask[:, j].unsqueeze(1).expand(-1, self.dim_flag[j]))
        # mask = torch.cat(a, dim=1).to(user_embed.device)
        # entity_res_emb0 = entity_embed * mask
        # entity_res_emb = torch.add(entity_res_emb, entity_res_emb0)
        # """---------------mask-------------------"""

        """agg user from diffirent atr"""
        user_embed_agg = self.item2user(graph_UIS, entity_embed, entity_res_emb, user_embed, user_like)
        if dropout:
            user_embed_agg = self.dropout(user_embed_agg)

        user_emb_f = torch.add(user_embed_agg, user_embed)
        entity_res_emb = torch.add(entity_res_emb, entity_embed)
...
class GraphConv_KGA0_atr2(nn.Module):
    ...
    def forward(self, graph, graph_UIS, all_embed, all_embed_cf, dropout):
        ...
        if dropout:
            # subg = {}
            # for edge in graph._canonical_etypes:
            #     subg[edge] = np.random.choice(np.arange(self.r_num0[edge]), size=int(self.r_num0[edge] * 0.9),
            #                          replace=False)
            # graph = dgl.edge_subgraph(graph, subg,preserve_nodes=True)
...
class GraphConv_KGA0_atrcat(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, dim_flag, ratedrop):
        super(GraphConv_KGA0_atrcat, self).__init__()
        ...
        self.item2user = KGA2_atr0cat(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                      n_relations=n_relations,
                                      dim=dim, dim_flag=dim_flag, dim_flag1=self.dim_flag1)

    def forward(self, graph, graph_UIS, all_embed_cf, all_embed, W_R, user_like, dropout):

        user_embed = all_embed[:self.n_users, :]
        entity_embed = all_embed[self.n_users:self.n_users + self.n_entities, :]

        user_embed_cf = all_embed_cf[:self.n_users, :]
        entity_embed_cf = all_embed_cf[self.n_users:self.n_users + self.n_entities, :]
        ...
        # norm_entity = []
        # for i in range(self.n_relations):
        #     norm_entity.append(F.normalize(entity_res_emb[:, self.dim_flag1[i]:self.dim_flag1[i + 1]], dim=1))
        # entity_res_emb = torch.cat(norm_entity, dim=1)

        """agg user from diffirent atr"""
        # user_embed_agg = self.item2user(graph_UIS, entity_embed_cf, entity_res_emb, user_like)
        user_embed_agg = self.item2user(graph_UIS, entity_embed_cf, entity_res_emb, user_embed, user_like)
        if dropout:
            user_embed_agg = self.dropout(user_embed_agg)

        user_emb_f = user_embed_agg + torch.cat([user_embed, user_embed_cf], dim=1)
        entity_res_emb = torch.cat([entity_res_emb, entity_embed_cf],
                                   dim=1)
...
class GraphConv_KGA00(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, dim_flag, ratedrop):
        super(GraphConv_KGA00, self).__init__()
        ...
        self.tri_embed = KGA001(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                n_relations=n_relations, dim=dim, dim_flag1=self.dim_flag1)
        ...
        self.item2user = KGA20(n_users=n_users, n_entities=n_entities, n_items=n_items,
                               n_relations=n_relations,
                               dim=dim, dim_flag=dim_flag)

    def forward(self, graph, graph_UIS, all_embed, all_embed0, W_R, user_like, dropout):
        # user_embed = []
        # entity_embed = []
        # for i in range(self.n_relations):
        #     user_embed.append(all_embed[i][:self.n_users, :])
        ...
        user_embed_0 = all_embed0[:self.n_users, :]
        entity_embed_0 = all_embed0[self.n_users:self.n_users + self.n_entities, :]
        ...
        entity_res_emb = torch.cat([entity_res_emb, entity_embed_0], dim=1)

        """agg user from diffirent atr"""
        user_embed_agg = self.item2user(graph_UIS, entity_res_emb, user_embed, user_like)
        user_emb_f = user_embed_agg + torch.cat([user_embed, user_embed_0], dim=1)
        # user_embed_agg = user_embed_agg + user_embed

        # if dropout:
        #     user_embed_0 = self.dropout(user_embed_0)
        #     entity_embed_0 = self.dropout(entity_embed_0)

        # user_emb_f = torch.cat([user_embed_0, user_embed_agg], dim=1)
        # entity_emb_f = torch.cat([entity_embed_0, entity_res_emb], dim=1)
...
class GraphConv_KGA1(nn.Module):

    def __init__(self, dim, n_hops, n_users, n_items, n_entities, n_relations, dim_flag, ratedrop):
        super(GraphConv_KGA1, self).__init__()
        ...
        self.tri_embed = KGA000(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                n_relations=n_relations, dim=dim, dim_flag1=self.dim_flag1)
...
class GraphConv_KGE(nn.Module):

    def __init__(self, dim, n_hops, n_hops1, n_users, n_items, n_entities, n_relations, num_like, ratedrop):
        super(GraphConv_KGE, self).__init__()

        self.tri_embed = KGE0(n_users=n_users, n_entities=n_entities, n_items=n_items,
                              n_relations=n_relations, dim=dim)
        ...
        for j in range(n_hops):
            self.convs_ui0.append(
                KGE1(n_users=n_users, n_entities=n_entities, n_items=n_items,
                     n_relations=n_relations,
                     dim=dim))

        self.dropout = nn.Dropout(p=0.1)  # mess dropout

        for j in range(n_hops1):
            self.UI.append(KGE2(n_users=n_users, n_entities=n_entities, n_items=n_items,
                                n_relations=n_relations,
                                dim=dim))

    def forward(self, graph, graph_UIS, all_embed, W_R, dropout):
        user_embed = all_embed[:self.n_users, :]
        entity_embed = all_embed[self.n_users:self.n_users + self.n_entities, :]
        # W_R = all_embed[self.n_users + self.n_entities:, :]
        ...
        item_emd = entity_res_emb[:self.n_items]
        item_res_emb = item_emd

        user_emb_f = user_embed

        """agg user from diffirent atr"""
        for j in range(self.n_hops1):
            item_emd, user_embed = self.UI[j](graph_UIS, item_emd, user_embed)
            item_res_emb = torch.add(item_res_emb, item_emd)
            user_emb_f = torch.add(user_emb_f, user_embed)

        return item_res_emb, user_emb_f
...
class GraphConv_KGE1(nn.Module):

    def __init__(self, dim, n_hops, n_hops1, n_users, n_items, n_entities, n_relations, num_like, ratedrop):
        super(GraphConv_KGE1, self).__init__()

        self.tri_embed = KGE10(n_users=n_users, n_entities=n_entities, n_items=n_items,
                               n_relations=n_relations, dim=dim)
        ...
        for j in range(n_hops):
            self.convs_ui0.append(
                KGE11(n_users=n_users, n_entities=n_entities, n_items=n_items,
                      n_relations=n_relations,
                      dim=dim))

        self.dropout = nn.Dropout(p=0.1)  # mess dropout

        self.UI = KGE12(n_users=n_users, n_entities=n_entities, n_items=n_items,
                        n_relations=n_relations,
                        dim=dim)

    def forward(self, graph, graph_UIS, all_embed, W_R, user_dict, user, dropout):
        user_embed = all_embed[:self.n_users, :]
        entity_embed = all_embed[self.n_users:self.n_users + self.n_entities, :]
        # W_R = all_embed[self.n_users + self.n_entities:, :]
        ...
        user_embed = self.UI(graph_UIS, user_dict, entity_res_emb, user_embed, user)
        user_emb_f = torch.add(user_emb_f, user_embed)
...
class RS_UIFKAN(nn.Module):
    def __init__(self, data_config, args_config):
        ...
        self.W_R = nn.Parameter(self.W_R)
        self.user_like = nn.Parameter(self.user_like)
        self.disen_weight_att = nn.Parameter(self.disen_weight_att)
    ...
    def _init_weight(self):
        ...
        self.W_R = initializer(torch.empty(self.n_relations, self.emb_size), gain=gain)
        self.user_like = initializer(torch.empty(self.n_users, self.num_like), gain=gain)

        self.disen_weight_att = initializer(torch.empty(self.num_like, self.n_relations))
    ...
    def forward(self, batch, graph, graph_nor, graph_UIS):
        # self.disen_weight_att = nn.Sigmoid()(self.disen_weight_att)
        user = batch['users']
        pos_item = batch['pos_items']
        neg_item = batch['neg_items']
        ...
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed,
                                              self.W_R, self.disen_weight_att, self.user_like, True)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
    ...
    def _cul_cor(self):
        disen_weight_att = nn.Sigmoid()(self.disen_weight_att)
    ...
    def generate(self, graph, graph_nor, graph_UIS):
        return self.gcn(graph, graph_nor, graph_UIS, self.all_embed,
                        self.W_R, self.disen_weight_att, self.user_like, False)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
    ...
    def create_bpr_loss(self, users, pos_items, neg_items, cor):
        ...
        #                             self.W_R.unsqueeze(0), dim=2))
        # loss1 = loss1 + loss11 + loss12
        loss2 = self.sim_decay * cor

        return mf_loss + emb_loss + loss2, mf_loss, cor


class RS_KGA(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGA, self).__init__()
        ...
        # 生成各边dim
        r_num = self.r_num / np.sum(self.r_num)
        cs = r_num * self.emb_size
        cs0 = np.around(cs)
        idx = np.where(cs0 < self.emb_size1)[0]
        cs0[idx] = self.emb_size1
        self.dim_f = int(np.sum(cs0))
        self.dim_flag = np.array(cs0, dtype=np.int).tolist()
    ...
    def _init_weight(self):
        ...
        self.all_embed = initializer(torch.empty(self.n_nodes, self.dim_f), gain=gain)
        ...
        self.user_like = initializer(torch.empty(self.n_users, self.n_relations), gain=gain)

    def _init_model(self):
        return GraphConv_KGA(dim=self.emb_size,
                             n_hops=self.context_hops,
                             n_users=self.n_users,
                             n_items=self.n_items,
                             n_entities=self.n_entities,
    ...
    def generate(self, graph, graph_UIS):
        return self.gcn(graph, graph_UIS, self.all_embed,
                        self.W_R, self.user_like, False)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
        # user_gcn_emb = user_gcn_emb - 1 * F.normalize(self.dis_embed, dim=1)
        # return item_gcn_emb, user_gcn_emb
...
class RS_KGA0(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGA0, self).__init__()
        ...
        # k = (self.emb_size=64 - self.emb_size1=1) / 5000
        # k = (self.emb_size - self.emb_size1) / 5000
        # b = self.emb_size1
        # cs1 = np.around(k * self.r_num + b)
        # cs1 = np.array(cs1, dtype=np.int)
        ...
        k = self.emb_size / 5000
        cs1 = np.ceil(k * self.r_num)
        cs1 = np.array(cs1, dtype=np.int)
        idx1 = np.where(cs1 > self.emb_size)
        cs1[idx1] = self.emb_size
        idx1 = np.where(cs1 == 0)
    ...
    def _init_weight(self):
        ...
        # self.all_embed = []
        # for i in range(self.n_relations):
        #     self.all_embed.append(initializer(torch.empty(self.n_nodes, self.dim_flag[i]), gain=gain).to(self.device))
        #     self.all_embed[i] = nn.Parameter(self.all_embed[i])
    ...
    def _init_model(self):
        return GraphConv_KGA0(dim=self.emb_size,
                              n_hops=self.context_hops,
                              n_users=self.n_users,
                              n_items=self.n_items,
                              n_entities=self.n_entities,
...
class RS_KGA0_att(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGA0_att, self).__init__()
    ...
    def _init_weight(self):
        initializer = nn.init.xavier_uniform_
        initializer1 = nn.init.xavier_normal_
        initializer1 = torch.nn.init.normal_
        initializer2 = torch.nn.init.constant_
        gain = 1.414
        # gain = 1
        ...
        self.user_like = initializer(torch.empty(self.n_relations, self.dim_f), gain=gain)
        # self.user_like = initializer1(torch.empty(self.n_users, self.n_relations), gain=gain)
        # self.user_like = initializer1(torch.empty(self.n_users, self.n_relations))
        # self.user_like = initializer2(torch.empty(self.n_users, self.n_relations), 1)
        # self.user_like = initializer(torch.empty(1, self.dim_f), gain=gain)
        # self.user_like = nn.ParameterDict()
        # for i in range(self.n_relations):
        #     self.user_like[str(i)] = nn.Parameter(initializer(torch.empty(1, self.dim_flag[i]), gain=gain))
        # self.user_like.append(initializer(torch.empty(1, self.dim_flag[i]), gain=gain))
        # self.user_like[i] = nn.Parameter(self.user_like[i])
        # self.user_like = initializer(torch.empty(1, self.dim_f), gain=gain)

    def _init_model(self):
        return GraphConv_KGA0_atr(dim=self.emb_size,
                                  n_hops=self.context_hops,
                                  n_users=self.n_users,
                                  n_items=self.n_items,
                                  n_entities=self.n_entities,
...
class RS_KGA0_att2(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGA0_att2, self).__init__()
...
class RS_KGA0_attcat(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGA0_attcat, self).__init__()
        ...
        self.emb_size = args_config.dim
        self.emb_size1 = args_config.dim1
        self.emb_size2 = args_config.dim2
    ...
    def _init_weight(self):
        ...
        self.all_embed = initializer(torch.empty(self.n_nodes, self.dim_f), gain=gain)
        self.all_embed_cf = initializer(torch.empty(self.n_nodes, self.emb_size2), gain=gain)
    ...
    def _init_model(self):
        return GraphConv_KGA0_atrcat(dim=self.emb_size,
                                     n_hops=self.context_hops,
                                     n_users=self.n_users,
                                     n_items=self.n_items,
                                     n_entities=self.n_entities,
    ...
    def forward(self, batch, graph, graph_UIS):
        ...
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_UIS, self.all_embed_cf, self.all_embed,
                                              self.W_R, self.user_like, True)

        u_e = user_gcn_emb[user]
        pos_e, neg_e = item_gcn_emb[pos_item], item_gcn_emb[neg_item]
        return self.create_bpr_loss(user, u_e, pos_e, neg_e)

    def generate(self, graph, graph_UIS):
        return self.gcn(graph, graph_UIS, self.all_embed_cf, self.all_embed,
                        self.W_R, self.user_like, False)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
        # user_gcn_emb = user_gcn_emb - 1 * F.normalize(self.dis_embed, dim=1)
        # return item_gcn_emb, user_gcn_emb
    ...
    def create_bpr_loss(self, user_idx, users, pos_items, neg_items):
        batch_size = users.shape[0]
        pos_scores = torch.sum(torch.mul(users, pos_items), axis=1)
        neg_scores = torch.sum(torch.mul(users, neg_items), axis=1)
        ...
        # loss_k = 0
        # for i in range(self.n_relations):
        #     loss_k = loss_k - torch.mean(
        #         self.gcn.item2user.out[user_idx][:, i] * \
        #         torch.log(
        #             (1e-8 + nn.ReLU()(torch.sum(
        #                 torch.mul(users[:, self.dim_flag1[i]:self.dim_flag1[i + 1]],
        #                           pos_items[:, self.dim_flag1[i]:self.dim_flag1[i + 1]]), axis=1))) /
        #             (1e-7 + nn.ReLU()(torch.sum(
        #                 torch.mul(users[:, self.dim_flag1[i]:self.dim_flag1[i + 1]],
        #                           neg_items[:, self.dim_flag1[i]:self.dim_flag1[i + 1]]), axis=1))))
        #     )
        # loss_k = self.sim_decay * loss_k
...
class RS_KGA00(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGA00, self).__init__()
        ...
        cs1[idx1] = self.emb_size
        self.dim_f = int(np.sum(cs1))
        # self.dim_f = self.dim_f + self.emb_size2
        self.dim_flag = np.array(cs1, dtype=np.int).tolist()
        ...
        self._init_weight()
        self.all_embed = nn.Parameter(self.all_embed)
        self.all_embed0 = nn.Parameter(self.all_embed0)
        self.user_like = nn.Parameter(self.user_like)
    ...
    def _init_weight(self):
        ...
        self.all_embed = initializer(torch.empty(self.n_nodes, self.dim_f), gain=gain)
        self.all_embed0 = initializer(torch.empty(self.n_nodes, self.emb_size2), gain=gain)
    ...
    def _init_model(self):
        return GraphConv_KGA00(dim=self.emb_size,
                               n_hops=self.context_hops,
                               n_users=self.n_users,
                               n_items=self.n_items,
                               n_entities=self.n_entities,
    ...
    def forward(self, batch, graph, graph_UIS):
        ...
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_UIS, self.all_embed, self.all_embed0,
                                              self.W_R, self.user_like, True)
    ...
    def generate(self, graph, graph_UIS):
        return self.gcn(graph, graph_UIS, self.all_embed, self.all_embed0,
                        self.W_R, self.user_like, False)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
        # user_gcn_emb = user_gcn_emb - 1 * F.normalize(self.dis_embed, dim=1)
        # return item_gcn_emb, user_gcn_emb
...
class RS_KGA1(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGA1, self).__init__()
    ...
    def _init_weight(self):
        ...
        self.all_embed = initializer(torch.empty(self.n_nodes, self.emb_size), gain=gain)
        self.W_R = initializer(torch.empty(self.n_relations, self.emb_size, self.emb_size1), gain=gain)
        self.user_like = initializer(torch.empty(self.n_users, self.n_relations), gain=gain)

    def _init_model(self):
        return GraphConv_KGA1(dim=self.emb_size,
                              n_hops=self.context_hops,
                              n_users=self.n_users,
                              n_items=self.n_items,
                              n_entities=self.n_entities,
...
class RS_KGE(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGE, self).__init__()
        ...
        self.emb_size = args_config.dim
        self.context_hops = args_config.context_hops
        self.context_hops1 = args_config.context_hops1
        self.n_factors = args_config.n_factors
        self.node_dropout = args_config.node_dropout
        self.node_dropout_rate = args_config.node_dropout_rate
        self.mess_dropout = args_config.mess_dropout
    ...
    def _init_model(self):
        return GraphConv_KGE(dim=self.emb_size,
                             n_hops=self.context_hops,
                             n_hops1=self.context_hops1,
                             n_users=self.n_users,
                             n_items=self.n_items,
                             n_entities=self.n_entities,
                             n_relations=self.n_relations,
    ...
    def forward(self, batch, graph, graph_UIS):
        ...
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_UIS, self.all_embed, self.W_R, True)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
    ...
    def generate(self, graph, graph_UIS):
        return self.gcn(graph, graph_UIS, self.all_embed, self.W_R, False)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
        # user_gcn_emb = user_gcn_emb - 1 * F.normalize(self.dis_embed, dim=1)
        # return item_gcn_emb, user_gcn_emb
...
class RS_KGE1(nn.Module):
    def __init__(self, data_config, args_config):
        super(RS_KGE1, self).__init__()
    ...
    def _init_model(self):
        return GraphConv_KGE1(dim=self.emb_size,
                              n_hops=self.context_hops,
                              n_hops1=self.context_hops1,
                              n_users=self.n_users,
                              n_items=self.n_items,
    ...
    def forward(self, batch, graph, graph_UIS, user_dict):
        ...
        item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_UIS, self.all_embed, self.W_R, user_dict, user, True)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
    ...
    def generate(self, graph, graph_UIS):
        return self.gcn(graph, graph_UIS, self.all_embed, self.W_R, user_dict, False)
        # item_gcn_emb, user_gcn_emb = self.gcn(graph, graph_nor, graph_UIS, self.all_embed, self.W_R)
        # user_gcn_emb = user_gcn_emb - 1 * F.normalize(self.dis_embed, dim=1)
        # return item_gcn_emb, user_gcn_emb
