from .metrics import *
from .parser import parse_args, parse_args1
...
def test2(model, graph, graph_UIS, user_dict, n_params, args):
    global device
    # Ks = eval(args.Ks)
    device = torch.device("cuda:" + str(args.gpu_id)) if args.cuda else torch.device("cpu")
    # BATCH_SIZE = args.test_batch_size
    ...
    with torch.no_grad():
        model = model.eval()
        entity_gcn_emb, user_gcn_emb = model.generate(graph, graph_UIS)
