from .metrics import *
from .parser import parse_args, parse_args1, parse_args10, parse_args_atr
...
def test3(model, graph, graph_1, graph_i2u, graph_u2u, graph_i2u_0, graph_i2u_1, user_dict, n_params):
    result = {'precision': np.zeros(len(Ks)),
              'recall': np.zeros(len(Ks)),
              'ndcg': np.zeros(len(Ks)),
              'hit_ratio': np.zeros(len(Ks)),
    ...
    with torch.no_grad():
        ...
        if n_users // args.test_batch_size > 0:
            for i in tqdm(range(n_users // args.test_batch_size)):
                ...
                entity_gcn_emb, user_batch = model.generate(graph, graph_1, graph_i2u, graph_u2u, graph_i2u_0_sub,
                                                            graph_i2u_1,
                                                            sub_idx,
            ...
            entity_gcn_emb, user_gcn_emb = model.generate(graph, graph_i2u, graph_u2u, graph_i2u_0_sub,
                                                          graph_i2u_1,
                                                          sub_idx,
                                                          user_dict['train_user_set'])
